...
export const transformFile = (file: any, isMounted: boolean): ContentItem => ({
    ...
    type: file.type === "IMAGE" ? "image" : file.type === "VIDEO" ? "video" : (file.type === "AUDIO" || file.fileType?.startsWith("audio/")) ? "audio" : "playlist",
    size: formatBytes(file.size),
    duration: formatDuration(file.duration),
    thumbnail: file.type === "IMAGE" ? getUrl(file.url) : undefined,
    video: file.type === "VIDEO" ? getUrl(file.url) : undefined,
    audio: (file.type === "AUDIO" || file.fileType?.startsWith("audio/")) ? getUrl(file.url) : undefined,
