...
//   const onXhrSettled = useCallback(() => {
...
//   const startXhrUpload = useCallback(
//     (entry: FileEntry) => {
//       const formData = new FormData();
//       formData.append("file", entry.file);
...
//             prev.map((f) =>
//               f.id === entry.id
//                 ? { ...f, progress: pct, uploaded: e.loaded }
//                 : f
//             )
//           );
//         }
...
//         activeUploadsRef.current = Math.max(0, activeUploadsRef.current - 1);
//         if (activeUploadsRef.current === 0) setIsPageLoading(false);
//       });

//       const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || "";
//       xhr.open("POST", `${baseUrl}/content/upload-file`);
//       const token = getCookie("token");
//       if (token) xhr.setRequestHeader("authorization", token);
//       xhr.send(formData);
//     },
//     [onXhrSettled, setIsPageLoading]
//   );
...
//       setFiles((prev) => [...prev, ...entries]);
//       setIsPageLoading(true);
//       entries.forEach((entry) => startXhrUpload(entry));
//     },
//     [startXhrUpload, setIsPageLoading]
//   );
...
//     // No files selected — just close
//     if (files.length === 0) {
//       onClose();
//       return;
//     }

//     // Files that need to be uploaded via mutation (pending/error)
//     const toUpload = files.filter(
//       (f) => f.status === "pending" || f.status === "error"
//     );

//     // If nothing left to upload (XHR already handled everything)
...
//     // Upload pending/error files via RTK mutation with progress tracking
//     setIsPageLoading(true);
...
//         // Mark this file as uploading in UI
//         setFiles((prev) =>
//           prev.map((f) =>
...
//         } catch (fileError: any) {
//           // Mark this individual file as error but continue loop
//           setFiles((prev) =>
//             prev.map((f) =>
//               f.id === entry.id ? { ...f, status: "error" } : f
//             )
...
//       // After all files attempted — refresh content list
//       dispatch(baseApi.util.invalidateTags(["Content"] as any));
//       onClose();
//     } catch (unexpectedError: any) {
//       toast.error(
//         unexpectedError?.data?.error?.message ||
...
//       setIsPageLoading(false);
//     }
//   }, [files, uploadFile, dispatch, onClose, setIsPageLoading]);
...
//   const hasFiles = files.length > 0;
//   const isUploading = files.some((f) => f.status === "uploading");
//   const allSettled = hasFiles && files.every(
...
//                             <div
//                               className={`h-full rounded-full transition-all duration-300 ${
//                                 isError ? "bg-red-400" : "bg-violet-500"
//                               }`}
//                               style={{ width: `${entry.progress}%` }}
//                             />
//                           </div>
...
//                       <button
//                         onClick={() => handleRemove(entry.id)}
//                         className="shrink-0 text-gray-400 hover:text-red-500 transition-colors mt-0.5 p-0.5 cursor-pointer"
//                       >
//                         <Trash2 className="w-4 h-4" />
//                       </button>
//                     </div>
...
//           <button
//             onClick={onClose}
//             className="flex-1 py-2.5 border border-border rounded-xl text-sm font-medium text-body hover:bg-gray-50 transition-colors shadow-customShadow cursor-pointer"
//           >
//             Cancel
//           </button>
//           <button
...
//             }`}
//           >
//             {isUploading ? "Uploading..." : allSettled ? "Done" : "Done"}
//           </button>
//         </div>
//       </div>
//     </div>
