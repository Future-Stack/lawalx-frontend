...
export default function AdminPreviewDeviceModal({ isOpen, onClose, device }: Props) {
  ...
  const timeline: any[] = useMemo(() => {
    const rawTimeline = currentDevice?.program?.timeline || [];
    if (rawTimeline.length > 0) return rawTimeline;

    // Fallback if there's only a single composed video available
    if (currentDevice?.program?.videoUrl) {
      return [{
        id: "composed-video",
        duration: currentDevice?.program?.duration || 0,
        file: {
          id: "file-1",
          url: currentDevice.program.videoUrl,
          originalName: currentDevice.program.name || "Composed Program Video",
          type: "VIDEO"
        }
      }];
    }
  ...
  const parseStorage = (storage: any, usedStorage?: number) => {
    if (usedStorage !== undefined) {
      const totalGB = typeof storage === 'number' ? storage : 10;
      const usedGB = Math.abs(usedStorage);
      return {
        used: usedGB,
        total: totalGB,
  ...
  const storageInfo = useMemo(() => {
    return parseStorage(
      ...
      currentDevice?.user?.usedStorage || currentDevice?.totalUsedStorage
    );
  }, [currentDevice]);
