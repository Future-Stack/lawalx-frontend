...
import { downloadSubscriberInvoicePdf } from "./_utils/downloadBillingInvoicePdf";
...
const TransactionSheet = ({ open, setOpen, userId }: TransactionSheetProps) => {
  ...
  const refundPaymentData: PaymentHistoryItem | null =
    payment && user
      ? {
  ...
  const handleDownload = async () => {
    ...
    try {
      await downloadSubscriberInvoicePdf(
        invoiceData,
        invoiceData.payment?.currency || "USD",
      );
    } catch (err) {
                  ...
                  <div className="grid grid-cols-[1fr_auto] gap-y-4 gap-x-3 text-sm">
                    ...
                    <div className="text-right font-medium flex items-center justify-end gap-1 text-body break-all">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          copyText(payment?.invoiceNumber || "N/A")
                        }
                  ...
                  <div className="grid grid-cols-[1fr_auto] gap-y-4 gap-x-3 text-sm">
                    ...
                    <div className="text-right font-medium flex items-center justify-end gap-1 text-body text-xs break-all">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          copyText(payment?.transactionId || "N/A")
                        }
                        className="inline-flex items-center"
