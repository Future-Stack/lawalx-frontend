...
import {
  ...
  SupporterRole,
  EmployeeRole,
} from "@/redux/api/admin/support/adminSupporterApi";
...
export default function AddEmployeeDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] p-0 bg-white dark:bg-gray-900 border-none shadow-2xl overflow-visible [&>button]:hidden">
        ...
        <form
          ...
          className="px-8 py-8 space-y-6 overflow-y-auto max-h-[calc(90vh-100px)] custom-scrollbar"
          ref={dropdownRef}
          autoComplete="off"
        >
          {/* Username Field */}
              ...
              <div className="flex flex-wrap gap-1.5">
                {formData.supporterRole.length > 0 ? (
                  formData.supporterRole.map((role) => (
                    <span
                      key={role}
                      className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg text-xs font-medium border border-blue-100 dark:border-blue-800"
                    >
                      {role.replace(/_/g, " ")}
                      <X
              ...
              <div className="absolute top-[calc(100%+4px)] left-0 w-full z-[100] bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl shadow-xl max-h-64 overflow-y-auto py-2 animate-in fade-in zoom-in-95 duration-150">
                {roles.map((role) => (
                  ...
                  >
                    <span
                      className={cn(
                        formData.supporterRole.includes(role)
            ...
            <div className="flex flex-wrap gap-2 min-h-[40px] p-3 bg-gray-50/50 dark:bg-gray-800/30 rounded-xl border border-dashed border-gray-200 dark:border-gray-700">
              {formData.skills.length > 0 ? (
                formData.skills.map((skill) => (
                  <span
                    key={skill}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 rounded-lg text-xs font-medium border border-green-100 dark:border-green-800 animate-in fade-in zoom-in duration-200"
                  >
                    {skill}
                    <button
                      type="button"
