...
const MOCK_EMPLOYEES: Employee[] = [
  {
    ...
    status: 'Available',
    activeTickets: 3,
    ticketTags: ['Authentication', 'Performance', 'Authentication', 'Billing'],
  },
  {
    id: '2',
    name: 'Sarah Johnson',
...
export default function TeamWorkloadDialog({
  ...
  const availableCount = MOCK_EMPLOYEES.filter((e) => e.status === 'Available').length;
  const totalActiveTickets = MOCK_EMPLOYEES.reduce((sum, e) => sum + e.activeTickets, 0);

  const displayedEmployees = showAvailableOnly
    ? MOCK_EMPLOYEES.filter((e) => e.status === 'Available')
    : MOCK_EMPLOYEES;
