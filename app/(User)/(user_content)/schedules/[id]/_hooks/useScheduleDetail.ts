...
export function useScheduleDetail() {
  ...
  const computedTimes = (() => {
    if (isOnce) {
      const totalDurationSeconds = calculateTotalDuration(content);
      const cleanStartDate = startDate.includes("T") ? startDate.split("T")[0] : startDate;
      const startDateTime = dayjs(`${cleanStartDate}T${startTime}:00`);
  ...
  const endDate =
    computedTimes?.endDate ??
    localEndDate ??
    (schedule?.endDate ? parseDate(schedule.endDate) : "");
  ...
  const getPayload = (
    ...
    const totalDurationSeconds = calculateTotalDuration(content);
    const startDateTimeStr = formatPayloadTime(pStartTime, startD);
    const startDateTime = dayjs(startDateTimeStr.replace("Z", ""));
    const endDateTime = startDateTime.add(totalDurationSeconds, "second");

    const computedEndDateStr = endDateTime.format("YYYY-MM-DD");
    const computedEndTimeStr = endDateTime.format("HH:mm:ss");

    const endD = isOnce ? computedEndDateStr : pEndDate || startD;
    ...
    return {
      ...
      endDate: isOnce
        ? formatPayloadDate(computedEndDateStr, true)
        : endD ? formatPayloadDate(endD, true) : undefined,
      startTime: startDateTimeStr,
      endTime: isOnce
        ? `${computedEndDateStr}T${computedEndTimeStr}Z`
        : formatPayloadTime(pEndTime, endD),
