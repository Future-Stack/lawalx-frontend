...
const AddContentDialog: React.FC<AddContentDialogProps> = ({
    ...
    const handleAddContent = () => {
        if (!data?.data || selectedItems.length === 0) return;
        const selectedFiles = (data.data as FileItem[]).filter((file: FileItem) =>
    ...
    const filteredContent = useMemo(() => {
        ...
        return (data.data as FileItem[]).filter((item: FileItem) => {
            ...
            const notAlreadyAdded = !existingFileIds.includes(item.id);
    ...
    return (
        ...
        >
            ...
            <div className="relative w-full max-w-[640px] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl flex flex-col overflow-hidden max-h-[90vh] border border-gray-200 dark:border-gray-700">
                {/* Header */}
                <div className="flex items-center justify-between px-5 sm:px-6 py-4 sm:py-5 border-b border-gray-200 dark:border-gray-700 shrink-0">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                            Choose videos or images to add to your schedule
                        </p>
                    </div>
                    <button
                        onClick={onClose}
