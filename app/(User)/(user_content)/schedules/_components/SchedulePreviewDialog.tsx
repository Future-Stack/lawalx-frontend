...
const SchedulePreviewDialog: React.FC<SchedulePreviewDialogProps> = ({
  ...
  const effectiveLowerThird =
    activeSchedule?.lowerThird ||
    (activeSchedule?.lowerThirds && activeSchedule.lowerThirds.length > 0
      ? activeSchedule.lowerThirds[0]
      : undefined);
  ...
  const handlePowerClick = async () => {
    ...
    try {
      ...
      toast.error(
        err?.data?.message || err?.error || "Failed to update schedule status",
      );
    }
  };
  ...
  const allItems = useMemo(() => {
    if (!activeSchedule) return [];
    const files = (activeSchedule.files || []).map((f) => ({
      ...f,
      isFile: true,
    }));
    const programs = (activeSchedule.programs || []).map((p) => ({
      ...p,
      isProgram: true,
    }));
  ...
  const shouldShowLowerThird = Boolean(
    currentItem && effectiveLowerThird?.text,
  );

  // Preload next item — browser starts fetching the next video/image before we switch
  const nextIndex = allItems.length > 1 ? (playingIndex + 1) % allItems.length : -1;
  const nextItem = nextIndex >= 0 ? allItems[nextIndex] : null;
  const nextMediaUrl = nextItem ? getUrl((nextItem as any).url || (nextItem as any).videoUrl || "") : null;
  ...
  const advance = useCallback(() => {
    ...
    const isOnce = activeSchedule?.recurrenceType?.toLowerCase() === "once";
    const isLastItem = playingIndex === allItems.length - 1;
  ...
  }, [allItems.length, playingIndex, activeSchedule?.recurrenceType]);

  useEffect(() => {
    if (allItems.length < 1 || !open || !localActive) return;

    const isOnce = activeSchedule?.recurrenceType?.toLowerCase() === "once";
    if (allItems.length === 1 && !isOnce) return;
    ...
    if (item.isProgram || item.type === "VIDEO" || item.type === "AUDIO")
      return;

    // Default to 7s for non-video items
    const duration = (currentItem as any).duration
      ? (currentItem as any).duration * 1000
      : 7000;
  ...
  const getPreviewContent = () => {
    ...
    if ((currentItem as any).isProgram) {
      return (
        <BaseVideoPlayer
          key={`${(currentItem as any).id}-${playbackVersion}`}
  ...
  return (
    <BaseDialog
      ...
      description={
        ...
        "View and manage this schedule's live playback settings."
      }
      maxWidth="4xl"
      maxHeight="2xl"
    ...
    >
      <div className="flex flex-col py-2 gap-4">
        ...
        <div className="w-full rounded-2xl bg-bgGray dark:bg-gray-800 shadow-sm aspect-video bg-black overflow-hidden relative group">
          ...
          <div
            className={cn(
              ...
              shouldShowLowerThird && effectiveLowerThird?.position !== "Top"
                ? "pb-12"
                : "",
            )}
          >
          ...
          {shouldShowLowerThird && (
            <div className="absolute inset-0 pointer-events-none flex flex-col justify-between z-10">
              ...
              {effectiveLowerThird?.text &&
                effectiveLowerThird.position === "Top" && (
                  <div
                    ...
                    style={{
                      backgroundColor: `${effectiveLowerThird.backgroundColor}${Math.round(
                        parseInt(
                          effectiveLowerThird.backgroundOpacity || "80",
                        ) * 2.55,
                      )
                        .toString(16)
                        .padStart(2, "0")}`,
                  ...
                  >
                    <Marquee
                      ...
                      direction={
                        effectiveLowerThird.animation === "Right_to_Left"
                          ? "left"
                          : "right"
                      }
                      gradient={false}
                    ...
                    >
                      <p
                        ...
                        style={{
                          ...
                          fontSize:
                            effectiveLowerThird.fontSize === "Small"
                              ? "14px"
                              : effectiveLowerThird.fontSize === "Medium"
                                ? "16px"
                                : "20px",
        ...
        <div className="flex flex-wrap items-center justify-between gap-x-6 gap-y-4 text-sm text-muted py-3">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">
              {getRecurrenceLabel()} •{" "}
              {formatTime(activeSchedule?.startTime || "")}
              {activeSchedule?.recurrenceType !== "once" &&
        ...
        <div className="flex items-center justify-end gap-3 pt-6 border-t border-border">
          ...
          <button
            onClick={() =>
              activeSchedule && handleDeleteSchedule(activeSchedule)
            }
